...
#define IMPLEMENT_CURL_OPTION(FUNCTION_NAME, OPTION_NAME, OPTION_TYPE) \
    ...
    void FUNCTION_NAME(OPTION_TYPE arg, boost::system::error_code& ec) \
    { \
...
#define IMPLEMENT_CURL_OPTION_BOOLEAN(FUNCTION_NAME, OPTION_NAME) \
    void FUNCTION_NAME(bool enabled) \
    { \
        boost::system::error_code ec; \
        FUNCTION_NAME(enabled, ec); \
        boost::asio::detail::throw_error(ec, BOOST_PP_STRINGIZE(FUNCTION_NAME)); \
    } \
    void FUNCTION_NAME(bool enabled, boost::system::error_code& ec) \
    { \
...
#define IMPLEMENT_CURL_OPTION_ENUM(FUNCTION_NAME, OPTION_NAME, ENUM_TYPE, OPTION_TYPE) \
    void FUNCTION_NAME(ENUM_TYPE arg) \
    { \
        boost::system::error_code ec; \
        FUNCTION_NAME(arg, ec); \
        boost::asio::detail::throw_error(ec, BOOST_PP_STRINGIZE(FUNCTION_NAME)); \
    } \
    void FUNCTION_NAME(ENUM_TYPE arg, boost::system::error_code& ec) \
    { \
...
#define IMPLEMENT_CURL_OPTION_STRING(FUNCTION_NAME, OPTION_NAME) \
    ...
    void FUNCTION_NAME(const char* str, boost::system::error_code& ec) \
    { \
    ...
    void FUNCTION_NAME(const std::string& str, boost::system::error_code& ec) \
    { \
...
namespace curl
{
	...
	{
	public:
		using handler_type = std::function<void(const boost::system::error_code &err)>;
		...
		using xferinfo_function_t = int (*)(void *clientp, native::curl_off_t dltotal, native::curl_off_t dlnow, native::curl_off_t ultotal, native::curl_off_t ulnow);
		...
		enum netrc_t
		{
			netrc_optional = native::CURL_NETRC_OPTIONAL, netrc_ignored = native::CURL_NETRC_IGNORED, netrc_required = native::CURL_NETRC_REQUIRED
		};
		...
		enum httpauth_t
		{
			auth_basic = CURLAUTH_BASIC, auth_digest, auth_digest_ie, auth_gss_negotiate, auth_ntml, auth_nhtml_wb, auth_any, auth_any_safe
		};

		void set_http_auth(httpauth_t auth, bool auth_only)
		{
			boost::system::error_code ec;
		...
		void set_http_auth(httpauth_t auth, bool auth_only, boost::system::error_code &ec)
		{
		...
		void set_http_post(std::shared_ptr<form> form, boost::system::error_code &ec);
		...
		void set_headers(std::shared_ptr<string_list> headers, boost::system::error_code &ec);
		...
		void set_http200_aliases(std::shared_ptr<string_list> http200_aliases, boost::system::error_code &ec);
		...
		enum http_version_t
		{
			http_version_none = native::CURL_HTTP_VERSION_NONE, http_version_1_0 = native::CURL_HTTP_VERSION_1_0, http_version_1_1 = native::CURL_HTTP_VERSION_1_1
		};
		...
		void set_mail_rcpts(std::shared_ptr<string_list> mail_rcpts, boost::system::error_code &ec);
		...
		void set_quotes(std::shared_ptr<string_list> quotes, boost::system::error_code &ec);
		...
		enum ftp_ssl_auth_t
		{
			ftp_auth_default = native::CURLFTPAUTH_DEFAULT, ftp_auth_ssl = native::CURLFTPAUTH_SSL, ftp_auth_tls = native::CURLFTPAUTH_TLS
		};

		IMPLEMENT_CURL_OPTION_ENUM(set_ftp_ssl_auth, native::CURLOPT_FTPSSLAUTH, ftp_ssl_auth_t, long);
		enum ftp_ssl_ccc_t
		{
			ftp_ssl_ccc_none = native::CURLFTPSSL_CCC_NONE, ftp_ssl_ccc_passive = native::CURLFTPSSL_CCC_PASSIVE, ftp_ssl_ccc_active = native::CURLFTPSSL_CCC_ACTIVE
		};
		...
		enum ftp_file_method_t
		{
			ftp_method_multi_cwd = native::CURLFTPMETHOD_MULTICWD, ftp_method_no_cwd = native::CURLFTPMETHOD_NOCWD, ftp_method_single_cwd = native::CURLFTPMETHOD_SINGLECWD
		};
		...
		{
			if_modified_since = native::CURL_TIMECOND_IFMODSINCE, if_unmodified_since = native::CURL_TIMECOND_IFUNMODSINCE
		};
		...
		enum ip_resolve_t
		{
			ip_resolve_whatever = CURL_IPRESOLVE_WHATEVER, ip_resolve_v4 = CURL_IPRESOLVE_V4, ip_resolve_v6 = CURL_IPRESOLVE_V6
		};
		...
		enum use_ssl_t
		{
			use_ssl_none = native::CURLUSESSL_NONE, use_ssl_try = native::CURLUSESSL_TRY, use_ssl_control = native::CURLUSESSL_CONTROL, use_ssl_all = native::CURLUSESSL_ALL
		};
		...
		void set_resolves(std::shared_ptr<string_list> resolved_hosts, boost::system::error_code &ec);
		...
		void set_ssl_verify_host(bool verify_host)
		{
			boost::system::error_code ec;
		...
		void set_ssl_verify_host(bool verify_host, boost::system::error_code &ec)
		{
		...
		void set_share(std::shared_ptr<share> share, boost::system::error_code &ec);
		...
		void set_telnet_options(std::shared_ptr<string_list> telnet_options, boost::system::error_code &ec);
